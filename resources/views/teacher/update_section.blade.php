...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-8">
            <h1 class="m-0">Update {{$courseName}} / {{$subjectNameCode}} / {{$semester}} / {{ $section }}</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
          ...
          <div class="col-12 wrapping" style="display:flex; gap: 10px;">
            ...
            <div class="card limitScroll" style="flex:50%;height: 100%;">
              ...
              <div class="card-header">
                ...
                <h3 class="card-title" style="display:flex; justify-content:space-between; width: 100%">update student list to your subject<span>S.Y. {{$acads}}</span></h3>
              </div>
              ...
              <form id="myForm" method="POST" action="{{ route('teacher.updating_section_student', ['id' => $idko]) }}">
              @csrf
              <textarea name="checkingBoxs" id="checkingBoxs" style="display:none" cols="" rows=""></textarea>
                ...
                @foreach($students as $data)
                  ...
                  <tr id="{{$data->id}}filling">
                    ...
                    @endphp
                      ...
                      <td><input id='{{$data->id}}button' {{$checked}} type="checkbox" name="checkboxes[]" class="checkbox" onclick="checkIfselecteds()" value="{{ $data->id }}"></td>
                      ...
                      <td><label for="{{$data->id}}button">{{ $data->sectioning }}</label></td>
            ...
            function checkingUncheck(){
              ...
              if(tempvars.checked == true && !ArrayAll.hasOwnProperty(parseInt(tempvars.value))){
              ...
              function confirm(){
                  ...
                  `;
                    ...
                    Swal.fire({
                        ...
                        confirmButtonText: 'Save this update'
                        }).then((result) => {
                        if (result.isConfirmed) {
                            checkingUncheck();
                            var textareaValue = document.getElementById('checkingBoxs');
                            textareaValue.value = JSON.stringify(ArrayAll);
