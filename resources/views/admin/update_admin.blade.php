          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Edit {{ (empty($finding_user->firstname)) ? 'N/A' : $finding_user->firstname }}'s information</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
              <div class="card-body">
              <form id="myForm" method="POST" action="{{ route('admin.updating_admin', ['id' => $finding_user->owner_id]) }}">
              @csrf
                <div class="row">
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label for="employee_no">Employee no.<sup class="sup">*</sup></label>
                            <input type="text" onkeyup="validateInput(this)" class="form-control" id="employee_no" name="employee_no" placeholder="Employee no." value="{{ (empty($finding_user->employee_no)) ? 'N/A' : $finding_user->employee_no }}">
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label for="first_name">First name<sup class="sup">*</sup></label>
                            <input type="text" class="form-control" id="first_name" name="first_name" placeholder="First name" value="{{ (empty($finding_user->firstname)) ? 'N/A' : $finding_user->firstname }}">
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label for="middle_name">Middle name</label>
                            <input type="text" class="form-control" id="middle_name" name="middle_name" placeholder="{{ (empty($finding_user->middlename)) ? 'N/A' : $finding_user->middlename }}" value="{{ $finding_user->middlename }}">
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label for="last_name">Last name<sup class="sup">*</sup></label>
                            <input type="text" class="form-control" id="last_name" name="last_name" placeholder="Last name" value="{{ (empty($finding_user->lastname)) ? 'N/A' : $finding_user->lastname }}">
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="form-group">
                ...
                <div class="d-flex gap-flex">
                    <a href="{{ route('admin.index') }}"><button type="button" class="btn bg-gradient-secondary" id="submit_buttons" onclick="">Back</button></a>
                ...
                <script>
                    ...
                    function validateForm(){
                        ...
                        const gmail = document.getElementById("gmail");
                            ...
                            jQuery.ajax({
                                    ...
                                    url: "{{ url('admin/checking_employee_exist') }}",
                                    data: {
                    ...
                    function validateForm_second() {
                        var id_admin = '{{ $finding_user->owner_id }}';
                        ...
                        var currentEmployeeNo = '{{ $finding_user->employee_no }}';
                        ...
                        if(gmail != "" && employees != "" && value != "" && value2 != "" && email.value != "" ){
                            ...
                            else if (value.length < 2 || value.length > 30 || !/^[A-Za-z\s]+$/.test(value)) {
                                markAsInvalid(input);
                                modals("The first name should have a minimum of 5 characters and a maximum of 30 characters.");
                                isValid = false;
                            }
                            else if ((value1.length < 2 || value1.length > 30 || !/^[A-Za-z\s]+$/.test(value1)) && (value1.length != "")) {
                                markAsInvalid(input1);
                                modals("The middle name should have a minimum of 5 characters and a maximum of 30 characters.");
                                isValid = false;
                            }
                            else if (value2.length < 2 || value2.length > 30 || !/^[A-Za-z\s]+$/.test(value2)) {
                                markAsInvalid(input2);
                                modals("The last name should have a minimum of 5 characters and a maximum of 30 characters.");
                                isValid = false;
                            }
                            else if (email.value.length > 30 || email.value.length < 5) {
                                markAsInvalid(email);
                                modals("The username should have a minimum of 5 characters and a maximum of 30 characters.");
                                isValid = false;
                            }
                          ...
                          else if(emailExist && gmail != currentGmail){
                                markAsInvalid(gmail);
