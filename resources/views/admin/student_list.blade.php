...
@section('titlePage') CCSTNexus | student @endsection
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header" >
                <h3 class="card-title"><a href="{{ route('admin.add_student') }}"><button class="btn btn-block bg-gradient-primary">Add account</button></a></h3>
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example3" class="table-bordered table-hover responsive nowrap table table-bordered table-striped">
                    ...
                    <tr>
                      ...
                      <th class="invisible">Last name</th>
                    ...
                    @foreach($all_student as $data)
                      ...
                      @endif
                          ...
                          <td>@if(isset($studentCOurse["'" . intval($data->id) . "'"]))  {{ $studentCOurse["'" . intval($data->id) . "'"] }}@endif</td>
                          ...
                          <td>
                            <div class="d-flex gap-flex">
                              <a class="update a_gray"   title="Edit" href="{{ route('admin.update_student', ['id' => $data->id ]) }}">
                              ...
                              <a class="update a_gray"  title="{{$data->disabled == '0' ? 'Disable' : 'Enable'}}" href="{{ route('admin.disabling_student', ['id' => $data->id ]) }}">
                                <i class="{{$data->disabled == '0' ? 'fas disabling fa-ban' : 'disabling far fa-check-circle'}}  "></i>
                              </a>

                            </div>
                          </td>
                          <td class="invisible">{{ $data->student_no }}</td>
  ...
  $('[data-toggle="tooltip"]').tooltip()
    ...
    function checkingTrues(password, id){
      ...
      if(password != 'false'){
        ...
        var newPageURL = "{{ route('admin.delete_student', ['id' => 'PLACEHOLDER_ID', 'password' => 'PLACEHOLDER_PASS']) }}"
   ...
   function deletingAll(ids, password) {
        ...
        jQuery.ajax({
          ...
          url: "{{ url('admin/delete_multiple_student') }}",
          data: {
              password: password,
              id: ids,
          },
      ...
      function disablingAll(ids, password) {
        ...
        jQuery.ajax({
          ...
          url: "{{ url('admin/disable_multiple_student') }}",
          data: {
              password: password,
              id: ids,
          },
