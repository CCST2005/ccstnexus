...
@section('titlePage') CCSTNexus | academic programs @endsection
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Academic programs</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header" >
                <h3 class="card-title"><a href="{{ route('admin.add_track') }}"><button class="btn btn-block bg-gradient-primary">Add academic program</button></a></h3>
                
              </div>
              <form id="checkboxForm">
                  @csrf
              <div class="card-header" style="display:none" id="disable&delete" >
                  <div><span style="opacity:80%">Selected academic program: <span id="countSelected"></span></span></div>
                  <div class=" card-header-flex gap-flex">
                    <h3 class="card-title"><button type="button" class="btn btn-block bg-gradient-danger " onclick="deleteAll('<span>Deleting a academic program has implications on various components, including the removal of the associated curriculum and the deactivation of accounts for students linked to the deleted academic program.</span><br><br>Admin password', 'deletingAll')">Delete all</button></h3>
                      ...
                      @endif
                          ...
                          <td>
                            <div class="d-flex gap-flex">
                              <a class="update a_gray"   title="Edit" href="{{ route('admin.update_track', ['id' => $data->id ]) }}">
                                <i class="fas fa-pencil-alt"></i>
                              </a>
                              
                              <a class="update a_gray"   title="Delete" onclick="deleting_admin({{$data->id}}, '<span>Deleting a academic program has implications on various components, including the removal of the associated curriculum and the deactivation of accounts for students linked to the deleted academic program.</span><br><br>Admin password');deletingValsWarn()">
  ...
  $('[data-toggle="tooltip"]').tooltip()
    ...
    function checkingTrues(password, id){
      ...
      if(password != 'false'){
        ...
        var newPageURL = "{{ route('admin.delete_track', ['id' => 'PLACEHOLDER_ID', 'password' => 'PLACEHOLDER_PASS']) }}"
   ...
   function deletingAll(ids, password) {
        ...
        jQuery.ajax({
          ...
          url: "{{ url('admin/delete_multiple_track') }}",
          data: {
              password: password,
              id: ids,
          },
