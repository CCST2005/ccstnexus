...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Edit subject from {{$curiculumn->title}}</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Choose what subject to add/remove</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
              @if($errors->any())
...
@endif
              ...
              @csrf
                ...
                <input type="hidden" class="" id="courseID" value="{{ $previousID }}" name="courseID">
                        ...
                        <select class="tags_input" name="roles[]" id="roles" multiple>
                            ...
                            @foreach($subjects as $roles)
                                ...
                                @foreach($roles_registrar as $roles_registrarr)
                                    ...
                                    @endif
                                @endforeach
                                <option value="{{ $roles->id }}" {{$selected}} >{{$roles->sub_code}} - {{$roles->title}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="d-flex gap-flex">
                    ...
                    <a href="{{ route('admin.update_curriculum', ['id' => $curiculumn->id, 'previous' => $previousID, 'year' => $acadsNewVar]) }}"><button type="button" class="btn bg-gradient-secondary" id="submit_button" onclick="">Back</button></a>
                ...
                <script>
                    ...
                    function validateForm_second() {
                        ...
                        var selectedValues = Array.from(selectElement.selectedOptions).map(option => option.value);
                            ...
                            //     Swal.fire({
                            //         title: '',
                            //         text: "Are you sure you want to edit this subject/s?",
                            //         icon: 'question',
